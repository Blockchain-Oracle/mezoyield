...
4. **Strategy cards** in `/app/earn/strategies` show a small `47% of protocol veMEZO` line under the title for the most-used strategy and a quieter `Xx% of protocol veMEZO uses this` for the rest. Hides when protocol total is < 0.01 veMEZO (avoid divide-by-tiny noise).
