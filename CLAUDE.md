...
10. **#29 P1 (pre-push) — Drift renormalization inflated mapped-strategy share.** `useProtocolStrategyMix` rounded percentages of single-gauge strategies, then added `100 - sum` to the top entry to make them sum to 100. When delegate / multi-gauge strategies hold weight, that drift IS the unattributed slice — adding it back inflates the displayed share. **Fix forever:** when you can only attribute a subset of a total, render the sub-100 sum honestly. Renormalization is only valid when the underlying data IS the full 100.
11. **#29 P1 (pre-push) — Sentinel conversion rates fake live data.** `ProtocolEarningsChart` shipped a BTC toggle that multiplied MUSD by `0.0000095` ("not a live price feed" — and yet, on a credibility chart). Disclaimers don't redeem invented numbers in display surfaces. **Fix forever:** if there's no real oracle, the toggle doesn't exist. Same anti-slop rule as gauges/APYs applies to derived/converted values.
12. **#29 P1 (pre-push) — `catch {}` returning defaults hides storage failures.** `usePreferences` swallowed JSON.parse + localStorage exceptions silently. Quota / private-mode / corrupted-blob errors became "settings don't persist" with zero signal to the dev. **Fix forever:** always `console.warn` (or richer log) in the catch — returning defaults is fine for recovery, but the reason must surface.
13. **#29 P2 (post-push) — Log-scan windows that assume slow blocks.** `useLastVote` capped at 30 × 9999 blocks = ~300k. Fine at Mezo testnet's ~4s/block (covers 14 days), tight at 1–2s mainnet (collapses below one epoch). **Fix forever:** size historical log scans by *time*, not *blocks*. Either use timestamp-bounded queries or set the block cap to comfortably exceed `epoch_length / fastest_plausible_block_time`. Current cap is 120 chunks (~1.2M blocks).
